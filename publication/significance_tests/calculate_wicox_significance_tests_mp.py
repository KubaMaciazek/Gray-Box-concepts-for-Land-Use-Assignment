...
def calculate_wilcox_significance_tests_mp(analysed_combinations, base_combination, samples, measures, results_dir,
                                        summary_dir, seeds, significance_results_path):
    table_headers = []
    rows = {}
    ...
    for j, sublist in enumerate(analysed_combinations):
        for i, combination in enumerate(sublist):
            # if j < 2:
            #     comb = combination + ';100;50;50'
            # else:
            #     comb = combination + ';100;50;100'
            rows[combination] = [combination]
    ...
    for measure in measures:
        for sample in samples:
            ...
            if measure == 'igd':
                analysed_sample_combination_results = get_sample_combinations_igds(base_combination, sample,
                                                                                   summary_dir)
            else:
                for j, sublist in enumerate(analysed_combinations):
                    analysed_sample_combination_results.update(get_sample_combinations_hvs(base_combination,
                                                                                      sublist, sample,
                                                                                      results_dir[j], seeds))
